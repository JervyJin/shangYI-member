    ...
    <div class="banner">
      <van-cell-group class="border">
        ...
        <div class="van-cell van-field drop-container">
          <label>证件类型</label>
          <van-dropdown-menu>
            <van-dropdown-item v-model="info.IDCardType" :options="IDCardTypeLis" />
          </van-dropdown-menu>
        </div>
      ...
      <van-popup v-model="showAddr" position="bottom" :style="{ height: '40%' }">
        ...
        <div class="jinian" @click="gotojinianri">
          <div class="fl">
      ...
      <!-- 提交按钮 -->
      <van-button class="button" type="info" @click="save(valArr)">提交</van-button>
    </div>
  </div>
</template>
...
import areaListInfo from "../common/area.js";
...
export default {
  ...
  methods: {
    ...
    addrConfirm(val) {
      // console.log(val)
      this.valArr = val;
      this.info.CityName = val[0].name + "-" + val[1].name + "-" + val[2].name;
      this.showAddr = false;
...
this.addrConfirm();
      this.http
        .put("/api/services/app/CRMMemberMyInfoService/UpdateCRMMember", {
          ...
          Sex: this.info.Sex,
          ProvinceCode: this.valArr[0].code,
          CityCode: this.valArr[1].code,
          RegionCode: this.valArr[2].code,
          CityName: this.info.CityName, //省市区名称
          IDCardType:this.info.IDCardType,
          IDCardNo: this.info.IDCardNo, //证件号
