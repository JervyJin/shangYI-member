<template>
  <div>
    ...
    <van-button class="button" type="info" @click="save">确定</van-button>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    save() {
      ...
      this.http
        .post(
          "/api/services/app/CRMMemberMyInfoService/AddCRMMemberImportant",
          {
      ...
      this.http
        ...
        .then(res => {
          ...
          if (res.Result.Code === 0) {
            ...
            // console.log(JSON.stringify(res.Result.Data[0].Date).split(" ")[0])
          }
        });
    }
  },
...
/deep/ {
  ...
  .van-row--justify-space-between::after {
      content: '';
              position: absolute;
              bottom: -15px;
              top: auto;
...
————————————————
版权声明：本文为CSDN博主「不开飞机的舒克」的原创文章，遵循 CC 4.0 BY-SA 版权协议，转载请附上原文出处链接及本声明。
原文链接：https://blog.csdn.net/qq_42896120/article/details/87780861 //居中处理
    }
